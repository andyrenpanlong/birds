...
def save_pic(url, image_name):
    try:
        ...
        if image.mode == 'P':
            image = image.convert('RGB')
        # 保存图片
        image.save('./test/{}.jpg'.format(image_name))
        global num_downs
        num_downs += 1
        print('下载图片数量为：', num_downs)
    except:
        pass
...
def get_birds_list():
    ...
    with connection.cursor() as cursor:
        ...
        for u1 in results:
            all_nums += 1
            print('图片下载到：', all_nums)
            url = u1[0]
            image_name = u1[1]
            if image_name not in over_download_pics:
                print('图片下载到: {},图片名称为：{} 正在下载, 图片链接为：{}'.format(len(over_download_pics) + all_nums, image_name, url))
                if num_downs <= 100:
                    save_pic(url, image_name)
...
# 计算时间差
time_diff = specific_time - start_time
...
print(f"距离特定时间点的时间差为 {days} 天, {hours} 小时, {minutes} 分钟, {seconds} 秒")