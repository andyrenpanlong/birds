...
# 获取ip总页数
def get_ip_page(url):
    print('proxies spider kxdaili start ...', url)
    try:
        response = requests.get(url, headers=headers, verify=False)
        bs = BeautifulSoup(response.text, 'html5lib')
        a_arr = bs.select('.hot-product-content tr')
        for i in a_arr:
            if len(i.select('td')) > 4:
                if 'HTTP' in i.select('td')[3].text.strip():
                    proxies = 'http://{}:{}'.format(i.select('td')[0].text.replace('\r', '').replace('\n', '').replace('\t', '').strip(), i.select('td')[1].text.replace('\r', '').replace('\n', '').replace('\t', '').strip())
                    print(proxies)
                    # save_redis(proxies)
        next_page = int(url.split('/')[-1].replace('.html', '')) + 1
        print(next_page)
        if len(a_arr) > 0:
            url_2 = 'http://www.kxdaili.com/dailiip/1/{}.html'.format(next_page)
            get_ip_page(url_2)
    except:
        pass
